...
void afficherProgression(unsigned j, unsigned total) {
    string progressBar = "";
    for (unsigned k = 1 ; k <= (100*j)/total ; k += 2) {
        progressBar += '-';
    }
    for (unsigned k = (100*j)/total + 1 ; k < 100 ; k += 2) {
        progressBar += ' ';
    }
    system("cls");
    cout << '[' << progressBar << "] " << (100*j)/total+1 << '%' << endl;
}
...
int main(int argc, char *argv[]) {
    ...
    dicoPath = dicoPath.substr(0, dicoPath.find(".csv")) + ".dico";
