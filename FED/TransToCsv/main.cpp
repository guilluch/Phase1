...
string traitementRegleAsso(string str) {
    ...
    //On supprime les ; en fin de ligne
    /*while (text.find(";\r", 0) != string::npos) {
        text.replace(text.find(";\r", 0), 2 , "");
    }*/
    ...
    //text.replace(pos+1, text.find(']')- 1, "\"" + motsSansDoublons[stoi(text.substr(pos+1, text.find(']')))] + "\"");

    //cout << lines[0].substr(pos+3, lines[0].find(',', pos+3)-1) << endl;
    /*lines[0].replace(pos+1, lines[0].find(',')- 1, "\"" + motsSansDoublons[stoi(lines[0].substr(pos+1, lines[0].find(',', pos)-1))] + "\"");
    cout << pos << endl;
    pos = lines[0].rfind('"');
    cout << pos << endl;
    cout << lines[0].substr(pos+3, lines[0].find(',', pos) - pos + 1) << endl;*/
    //cout << lines[0] << endl;
    ...
    for (unsigned i = 0 ; i < lines.size() ; ++i) {
        ...
        while (lines[i].find(']'), pos+3) {
            if (lines[i].find(',', pos+3) < lines[i].find(']', pos)) {
                ...
                //cout << lines[i].substr(pos+3, lines[i].find(']', pos)-pos-3) << endl;
...
int main(int argc, char *argv[]) {
    ...
    string dicoPath = "science-en.dico";
